...
namespace {
    ...
    constexpr wchar_t kResizeHitWindowClassName[] =
        L"WebViewNativeShellResizeHitWindow";
    ...
    TaskbarSide detect_taskbar_side(const MONITORINFO& info, const POINT anchor) {
        ...
        if (anchor.y < info.rcWork.top && anchor.y >= info.rcMonitor.top)
            return TaskbarSide::Top;
        if (anchor.x < info.rcWork.left && anchor.x >= info.rcMonitor.left)
            return TaskbarSide::Left;
        ...
        const LONG nearest =
            std::min({ left_distance, right_distance, top_distance, bottom_distance });
        if (bottom_distance == nearest)
            return TaskbarSide::Bottom;
        if (top_distance == nearest)
            return TaskbarSide::Top;
        if (left_distance == nearest)
            return TaskbarSide::Left;
        return TaskbarSide::Right;
    }
    ...
    void configure_window_frame(const HWND window) {
        ...
        // Extend one pixel of the DWM frame so frameless host windows keep the native
        // shadow.
...
NativeShell::NativeShell(Napi::ThreadSafeFunction event_callback)
    ...
    webview_state_(std::make_unique<WebViewState>()) {
}

NativeShell::~NativeShell() { shutdown(); }
...
void NativeShell::open_window(WindowOpenOptions options) {
    post_command([this, options = std::move(options)]() mutable {
        open_window_on_ui(std::move(options));
        });
}
    ...
    post_command([this, message = std::move(message)] {
        if (webview_state_->webview) {
            ...
            const HRESULT result =
                webview_state_->webview->PostWebMessageAsString(wide.c_str());
            if (FAILED(result)) {
...
void NativeShell::execute_script(std::string script) {
    post_command([this, script = std::move(script)] {
        ...
        const HRESULT result =
            webview_state_->webview->ExecuteScript(wide.c_str(), nullptr);
        if (FAILED(result)) {
...
void NativeShell::open_path(std::wstring path) {
    post_command([this, path = std::move(path)] {
        const auto result = reinterpret_cast<std::intptr_t>(
            ShellExecuteW(message_window_, L"open", path.c_str(), nullptr, nullptr,
                SW_SHOWNORMAL));
        if (result <= 32) {
...
void NativeShell::run_ui_thread() {
    ...
    if (FAILED(com_result)) {
        ...
        startup_error_ =
            "初始化 Win32 STA 线程失败：" + hresult_message(com_result);
        ready_ = true;
    ...
    message_window_ = CreateWindowExW(
        WS_EX_TOOLWINDOW, kWindowClassName, L"WebView Native Shell", WS_POPUP,
        -32000, -32000, 1, 1, nullptr, nullptr, GetModuleHandleW(nullptr), this);
...
bool NativeShell::register_window_class() {
    window_icon_large_ = static_cast<HICON>(
        LoadImageW(nullptr, config_.icon_path.c_str(), IMAGE_ICON,
            GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON),
            LR_LOADFROMFILE));
    window_icon_small_ = static_cast<HICON>(
        LoadImageW(nullptr, config_.icon_path.c_str(), IMAGE_ICON,
    ...
    if (RegisterClassExW(&resize_hit_class) != 0 ||
        GetLastError() == ERROR_CLASS_ALREADY_EXISTS) {
        return true;
    }
...
HWND NativeShell::create_host_window(const WindowOpenOptions& options) {
    ...
    const int initial_height =
        scale_dimension(options.base_height, options.ui_scale_percent);

    const HWND window = CreateWindowExW(
        extended_style, kWindowClassName, options.title.c_str(), style,
        CW_USEDEFAULT, CW_USEDEFAULT, initial_width, initial_height, nullptr,
...
void NativeShell::show_tray_menu() {
    ...
    const UINT selected_command =
        TrackPopupMenu(menu, TPM_RETURNCMD | TPM_NONOTIFY | TPM_RIGHTBUTTON,
            cursor.x, cursor.y, 0, message_window_, nullptr);
    DestroyMenu(menu);
    PostMessageW(message_window_, WM_NULL, 0, 0);

    const auto match =
        std::find_if(mappings.begin(), mappings.end(),
            [selected_command](const CommandMapping& item) {
                return item.command == selected_command;
            });
...
void NativeShell::create_resize_hit_windows() {
    ...
    for (auto& resize_window : resize_hit_windows_) {
        resize_window =
            CreateWindowExW(WS_EX_TRANSPARENT, kResizeHitWindowClassName, L"",
                WS_CHILD | WS_VISIBLE, 0, 0, 0, 0, host_window_,
                nullptr, GetModuleHandleW(nullptr), this);
        if (!resize_window) {
...
void NativeShell::layout_resize_hit_windows() {
    ...
    const int border =
        std::max(1, dip_to_px(kResizeBorderDip, GetDpiForWindow(host_window_)));
    ...
    const std::array<RECT, 8> bounds = {
        RECT{0, vertical_border, horizontal_border,
             vertical_border + middle_height},
        RECT{width - horizontal_border, vertical_border, width,
             vertical_border + middle_height},
        RECT{horizontal_border, 0, horizontal_border + middle_width,
             vertical_border},
        RECT{horizontal_border, height - vertical_border,
             horizontal_border + middle_width, height},
    ...
    for (std::size_t index = 0; index < resize_hit_windows_.size(); ++index) {
        ...
        SetWindowPos(resize_hit_windows_[index], HWND_TOP, rect.left, rect.top,
...
void NativeShell::close_window_on_ui(const bool emit_closed) {
    ...
    if (webview_state_->webview && webview_state_->web_message_registered) {
        webview_state_->webview->remove_WebMessageReceived(
...
void NativeShell::begin_webview_creation(const std::uint64_t generation) {
    const HRESULT result = CreateCoreWebView2EnvironmentWithOptions(
        nullptr, config_.webview_data_directory.c_str(), nullptr,
        Callback<ICoreWebView2CreateCoreWebView2EnvironmentCompletedHandler>(
            [this, generation](HRESULT environment_result,
                ICoreWebView2Environment* environment) -> HRESULT {
                    if (generation != generation_ || !host_window_) {
                        return S_OK;
                    }
                    if (FAILED(environment_result) || !environment) {
                        emit_error("创建 WebView2 Environment 失败：" +
                            hresult_message(environment_result));
    ...
    if (FAILED(result)) {
        emit_error("请求创建 WebView2 Environment 失败：" +
...
void NativeShell::configure_webview(const std::uint64_t generation) {
    if (generation != generation_ || !webview_state_->webview ||
        !webview_state_->controller) {
        return;
    }
    ...
    const HRESULT mapping_result = webview3->SetVirtualHostNameToFolderMapping(
        kVirtualHost, config_.renderer_root.c_str(),
    ...
    const HRESULT navigation_event_result =
        webview_state_->webview->add_NavigationStarting(
            ...
            .Get(),
            &webview_state_->navigation_start_token);
    webview_state_->navigation_start_registered =
        SUCCEEDED(navigation_event_result);

    const HRESULT web_message_event_result =
        webview_state_->webview->add_WebMessageReceived(
    ...
    const HRESULT navigate_result =
        webview_state_->webview->Navigate(url.c_str());
...
void NativeShell::show_and_focus_window() {
    ...
    if (webview_state_->controller) {
        webview_state_->controller->MoveFocus(
...
void NativeShell::apply_window_scale(const int percent) {
    ...
    if (window_options_.resizable && old_factor > 0.0) {
        new_width_dip = std::max(1, static_cast<int>(std::lround(
            old_width_dip / old_factor * new_factor)));
        new_height_dip =
            std::max(1, static_cast<int>(
                std::lround(old_height_dip / old_factor * new_factor)));
    }

    const int width = dip_to_px(new_width_dip, dpi);
    const int height = dip_to_px(new_height_dip, dpi);
    SetWindowPos(host_window_, nullptr, 0, 0, width, height,
...
void NativeShell::position_anchor_window() {
    ...
    const int width = dip_to_px(
        scale_dimension(window_options_.base_width, ui_scale_percent_), dpi);
    const int height = dip_to_px(
        scale_dimension(window_options_.base_height, ui_scale_percent_), dpi);
    ...
    case TaskbarSide::Bottom:
        x = clamp_position(x, info.rcWork.left + margin,
    ...
    case TaskbarSide::Left:
        ...
        y = clamp_position(y, info.rcWork.top + margin,
    ...
    SetWindowPos(
        host_window_, window_options_.always_on_top ? HWND_TOPMOST : nullptr, x,
        y, width, height,
...
void NativeShell::position_bounds_window() {
    ...
    const UINT dpi = monitor_dpi(monitor);
    const int width =
        dip_to_px(scale_dimension(bounds.width, ui_scale_percent_), dpi);
    const int height =
        dip_to_px(scale_dimension(bounds.height, ui_scale_percent_), dpi);
    RECT target{ bounds.x, bounds.y, bounds.x + width, bounds.y + height };
    const HMONITOR visible_monitor =
        MonitorFromRect(&target, MONITOR_DEFAULTTONULL);

    if (!visible_monitor) {
        center_window_on_primary_monitor();
        return;
    }
    SetWindowPos(host_window_, nullptr, bounds.x, bounds.y, width, height,
...
void NativeShell::center_window_on_primary_monitor() {
    ...
    SetWindowPos(host_window_, nullptr, x, y, width, height,
        SWP_NOZORDER | SWP_NOACTIVATE);
}
...
void NativeShell::emit_window_bounds() {
    ...
    RECT window_rect{};
    RECT client_rect{};
    if (!GetWindowRect(host_window_, &window_rect) ||
        !GetClientRect(host_window_, &client_rect)) {
        return;
    }
    const UINT dpi = GetDpiForWindow(host_window_);
    const double factor = static_cast<double>(ui_scale_percent_) / 100.0;
    const int client_width_dip =
        px_to_dip(client_rect.right - client_rect.left, dpi);
    const int client_height_dip =
        px_to_dip(client_rect.bottom - client_rect.top, dpi);
    ...
    event.bounds.width =
        std::max(1, static_cast<int>(std::lround(client_width_dip / factor)));
    event.bounds.height =
        std::max(1, static_cast<int>(std::lround(client_height_dip / factor)));
    emit(std::move(event));
}
...
LRESULT CALLBACK NativeShell::window_proc(HWND window, UINT message,
    WPARAM wparam, LPARAM lparam) {
    NativeShell* shell =
        reinterpret_cast<NativeShell*>(GetWindowLongPtrW(window, GWLP_USERDATA));
...
LRESULT CALLBACK NativeShell::resize_hit_window_proc(HWND window, UINT message,
    WPARAM wparam,
    LPARAM lparam) {
    NativeShell* shell =
        reinterpret_cast<NativeShell*>(GetWindowLongPtrW(window, GWLP_USERDATA));
    ...
    if (shell) {
        return shell->handle_resize_hit_window_message(window, message, wparam,
            lparam);
    }
    return DefWindowProcW(window, message, wparam, lparam);
}

LRESULT NativeShell::handle_resize_hit_window_message(const HWND window,
    const UINT message,
    const WPARAM wparam,
    const LPARAM lparam) {
    ...
    case WM_LBUTTONDOWN:
        if (host_window_ && window_options_.resizable) {
            ...
            GetCursorPos(&cursor);
            ReleaseCapture();
            SendMessageW(host_window_, WM_NCLBUTTONDOWN, hit_test,
    ...
    if (window == host_window_) {
        ...
        case WM_GETMINMAXINFO:
            if (window_options_.resizable) {
                ...
                info->ptMinTrackSize.x = dip_to_px(
                    scale_dimension(window_options_.min_width, ui_scale_percent_), dpi);
                info->ptMinTrackSize.y = dip_to_px(
                    scale_dimension(window_options_.min_height, ui_scale_percent_),
                    dpi);
                return 0;
            }
            break;
        ...
        case WM_ACTIVATE:
            if (window_options_.close_on_deactivate &&
