...
Napi::Object require_object(const Napi::CallbackInfo& info, std::size_t index, const char* name) {
...
std::string get_string(const Napi::Object& object, const char* key) {
    ...
    if (!value.IsString()) {
        throw Napi::TypeError::New(object.Env(), std::string(key) + " 必须是字符串");
    }
...
bool get_bool(const Napi::Object& object, const char* key, bool fallback = false) {
    const auto value = object.Get(key);
    if (value.IsUndefined()) {
        return fallback;
    }
    if (!value.IsBoolean()) {
        throw Napi::TypeError::New(object.Env(), std::string(key) + " 必须是布尔值");
    }
...
WindowPlacement parse_placement(const Napi::Object& options) {
    ...
    throw Napi::TypeError::New(options.Env(), "placement 必须是 center、anchor 或 bounds");
}
...
Napi::Value initialize_shell(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        config.webview_data_directory = utf8_to_wide(get_string(options, "webviewDataDirectory"));
...
Napi::Value open_window(const Napi::CallbackInfo& info) {
    ...
    try {
        ...
        if (x.IsNumber() && y.IsNumber()) {
            window.anchor = POINT{x.As<Napi::Number>().Int32Value(), y.As<Napi::Number>().Int32Value()};
        }
    ...
    try {
        ...
        for (std::uint32_t index = 0; index < array.Length(); ++index) {
            ...
            if (type == "separator") {
                ...
                throw Napi::TypeError::New(env, "托盘菜单项 type 必须是 item 或 separator");
            }
